...
Split leakage. If a sample id shows up under more than one split value anywhere in the sheet, it's compromised. Drop it completely, its train rows included, and count it as leakage.
Blank label. Empty after trimming.
Malformed label. After trimming, a value with control characters, made entirely of punctuation, or an obvious placeholder like null, n/a, none, unknown, or tbd.
Conflicting label. A surviving sample id that appears on multiple train rows whose canonicalized labels don't all agree. Drop the whole id, don't pick a winner.
